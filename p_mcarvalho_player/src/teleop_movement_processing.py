...
pub = rospy.Publisher('red1/cmd_vel', Twist, queue_size=1)
joint_pub = rospy.Publisher('red1/joint_states', JointState,queue_size=1)
...
class Teleop_Server:
    ...
    def callbackTeleopReceived(self, msg):
        jointsate = JointState()
        jointsate.name = ["my_movable_joint"]
        jointsate.position = []
        jointsate.velocity = [0.1]
        jointsate.effort = []
        joint_pub.publish(jointsate)
        rospy.loginfo(jointsate)
        pub.publish(msg)        #rospy.loginfo(msg)
...
def main():
    ...
    jointsate = JointState()
    jointsate.name = ["my_movable_joint"]
    jointsate.position = [0]
    jointsate.velocity = [0.01]
    jointsate.effort = [0.01]
    joint_pub.publish(jointsate)
    rospy.loginfo(jointsate)


    rospy.Subscriber('red1/teleop_vel', Twist, callback=teleop_server.callbackTeleopReceived)
    #rospy.Subscriber('red1/scan', LaserScan, callback=teleop_server.callbackMessageReceived)
